...
open(991,file='profile-evp20-train.ascii')
...
if(stat ==0)then
do
read(1,*,iostat=stat)alat,alon,hgt,aa1,aa2,mon !bb1,bb2,cc1,cc2
if(stat /=0)exit
      icnt=icnt+1
      do jj=1,31
             if(hgt >= lev(jj) .and. hgt < lev(jj+1))then
             ixx(jj)=ixx(jj)+1
             var10(jj,ixx(jj))=34.6566*aa1
             var11(jj,ixx(jj))=34.6566*aa2
!            var20(jj,ixx(jj))=bb1
!            var21(jj,ixx(jj))=bb2
!            var30(jj,ixx(jj))=cc1
!            var31(jj,ixx(jj))=cc2
